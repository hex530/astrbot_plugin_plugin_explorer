...
@register("plugin_explorer", "夕小柠 & 陆渊", "智能插件管家：搜索、读文档并建议安装。", "1.1.0")
class PluginExplorer(Star):
    ...
    async def search_github_plugins(self, event: AstrMessageEvent, keyword: str):
        """
        在 GitHub 上搜索 AstrBot 插件，并尝试读取 README 进行筛选。
        参数 keyword: 搜索关键词。
        """
        if not self._is_admin(event):
            return "错误：只有我的主人熙熙才能让我去外面探险哦。"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                if resp.status != 200: return f"GitHub 搜索暂时不可用 (状态码: {resp.status})"
                data = await resp.json()
                items = data.get('items', [])[:3]
                
                if not items: return f"没搜到关于‘{keyword}’的 AstrBot 插件。"
                ...
                for item in items:
                    ...
                    async with session.get(readme_url) as r_resp:
                        if r_resp.status == 200:
                            readme_text = await r_resp.text()
                            readme_content = readme_text[:100].replace('\n', ' ') + "..."
